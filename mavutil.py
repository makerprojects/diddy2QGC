...
class mavudp(mavfile):
    ...
    def __init__(self, device, input=True, broadcast=False, source_system=255, source_component=0, use_native=default_native, timeout=0, **opts):
        a = device.split(':')
        if len(a) != 2:
        ...
        if input:
            ...
            if 'loclAddr' in opts:
                b = opts['loclAddr'].split(':')
                if len(b) != 2:
                    print("UDP ports must be specified as host:port")
                    sys.exit(1)
                self.port.bind((b[0], int(b[1])))
                # port.bind(('0.0.0.0',14551))
            if broadcast:
...
def mavlink_connection(device, baud=115200, source_system=255, source_component=0,
    ...
    if device.startswith('udpout:'):
        return mavudp(device[7:], input=False, source_system=source_system, source_component=source_component, use_native=use_native, **opts)
